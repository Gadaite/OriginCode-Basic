...
object trim_Sql extends App{
    override def main(args: Array[String]): Unit = {
        ...
        */
        ...
        val models_name = distinct_model.collect()//转为一行行的Row,再取出其中的内容查看
        ...
        //mothod 1:
        spark.sql("""
            select * from bmw where trim(model) = '5 Series'
            """).show()
        spark.sql("""
            select * from bmw where model like '%5%Series%'
            """).show()
